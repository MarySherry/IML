...
def function_to_optimize(alphas, y, x, kernel):
    ...
    for i in range(n_samples):
        for j in range(n_samples):
            f += 0.5 * y[i] * y[j] * alphas[i] * alphas[j] * kernel(x[i], x[j])
...
def find_support_vector_inds(alphas, thresh):
    # todo write your code here
    sv_inds = np.where(alphas > thresh)[0]
    return sv_inds
...
def find_w_b(alphas, y, x, sv_inds, kernel, thresh, C):
    ...
    if kernel == linear_kernel:
        ...
        for i in range(1, n_samples):
            if sv_y[i] == 1 and alphas[i] > thresh and alphas[i] < C:
                b_pos = np.dot(sv[i], w.T)
                break

        for i in range(1, n_samples):
            if sv_y[i] == -1 and alphas[i] > thresh and alphas[i] < C:
                b_neg = np.dot(sv[i], w.T)
                break
        ...
        for i in range(n_samples):
            b += sv_y[i]
            for j in range(n_samples):
                b -= alphas[j] * sv_y[j] * kernel(sv[i], sv[j])
        b /= n_samples
...
# sv_inds - indices of support vectors
# kernel - callable, defines kernel function to use
def predict(alphas, y, x, x_test, w, b, sv_inds, kernel):
    ...
    for i in range(len(x_test)):
        s = 0
        for a, s_y, s_x in zip(alphas, sv_y, sv):
            s += a * s_y * kernel(x_test[i], s_x)
        predictions[i] = s
...
def minimize_and_plot(X, Y, kernel, C, thresh):
    ...
    grid_predictions = predict(alphas, Y, X, grid, w, b, sv_inds, kernel)
